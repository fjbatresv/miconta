...
use MiConta\SoporteBundle\Form\Type\Perfil\EditType;
...
class EditController extends BaseController
{
    public function indexAction($pk)
    {
        ...
        return $this->render('MiContaSoporteBundle:PerfilEdit:index.html.twig', $this->getAdditionalRenderParameters($Perfil) + array(
            "Perfil" => $Perfil,
        ...
        if ($form->isValid()) {
            try {
                ...
                if($this->get('request')->request->has('save-and-list'))
                  ...
                  return new RedirectResponse($this->generateUrl("MiConta_SoporteBundle_Perfil_edit", array('pk' => $pk) ));
    ...
    {
        return $this->get('session')->get('MiConta\SoporteBundle\PerfilEdit\Versions', array());
    }
    
        protected function setVersions($versions = array())
    {
        $this->get('session')->set('MiConta\SoporteBundle\PerfilEdit\Versions', $versions);
    }
